...
static int tph_camera_sw_handler(const char *type, u32 args)
{
    ...
    if (!strcmp(type, "id")) {
        ...
        ui_hide(TEXT_REMAIN_NUM_TPH);
        ui_show(TEXT_REMAIN_NUM_TPH);
        /*
         * VGA
         */
        ui_hide(TEXT_RES_TPH);
...
static int tph_fs_err_handler(const char *type, u32 arg)
{
    if (fget_err_code(CONFIG_ROOT_PATH) == -EIO) {
        __tph_msg_show(BOX_MSG_SD_ERR, 3000);
    } else {
...
static int tph_remain_num_handler(const char *type, u32 arg)
{
    ...
    ui_text_set_str_by_id(TEXT_REMAIN_NUM_TPH, "ascii", remain_num_str);
    return 0;
}
...
static int tph_tell_app_exit_menu(int p)
{
...
void tph_exit_menu_post(void)
{
    ...
    ui_set_call(tph_tell_app_exit_menu, 0);
    __this->menu_status = 0;
}

static void video_photo_ui_event_handler(struct sys_event *event, void *priv)
{
...
int tph_mode_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        ...
        id = register_sys_event_handler(SYS_DEVICE_EVENT, 0, 0, video_photo_ui_event_handler);
