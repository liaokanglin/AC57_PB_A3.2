...
int cmd_get_uuid(void *priv, char *content)
{
    ...
    sprintf(buf, "uuid:%s%02x%02x%02x%02x%02x%02x", UUID, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    /*printf("\n\nUUID : %s \n\n",buf);*/
    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "UUID", "NOTIFY", buf);
    return 0;
}

static int get_video_rec_fps(u8 id)
{
    return 30;
}

static int get_video_rec_audio_rate(void)
{
    return 8000;
}
...
{
    char buf[128];
    int format = 1; //0 表示jpeg 1表示h264
    u32 res = db_select("res");
    switch (res) {
    case VIDEO_RES_1080P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 1920, 1080, format, get_video_rec_fps(0), get_video_rec_audio_rate());
        break;

    case VIDEO_RES_720P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 1280, 720, format, get_video_rec_fps(0), get_video_rec_audio_rate());
        break;

    case VIDEO_RES_VGA:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 640, 480, format,  get_video_rec_fps(0), get_video_rec_audio_rate());
        break;
...
int cmd_get_pull_video_param(void *priv,  char *content)
{
    ...
    switch (res) {
    case VIDEO_RES_1080P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 1920, 1080, 1, get_video_rec_fps(1), get_video_rec_audio_rate());
        break;

    case VIDEO_RES_720P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 1280, 720, 1,  get_video_rec_fps(1), get_video_rec_audio_rate());
        break;

    case VIDEO_RES_VGA:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 640, 480, 1,   get_video_rec_fps(1), get_video_rec_audio_rate());
        break;
    default:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 640, 480, 1,   get_video_rec_fps(1), get_video_rec_audio_rate());
        break;
...
int cmd_put_format(void *priv, char *content)
{
    ...
    net_video_rec_post_msg("Remain:s=%4", ((u32)(second)));
#endif
...
{
    char buf[32];  // 定义一个字符数组，用于存储语言信息
    u32 lag = db_select("lag");  // 从数据库中获取当前语言设置
    sprintf(buf, "lag:%d", lag);  // 将语言设置格式化为字符串

    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "LANGUAGE", "NOTIFY", buf);  // 发送语言通知
    return 0;  // 返回0表示成功
}

extern int lag_set_function(u32 parm);  // 声明外部函数，用于设置语言功能
int cmd_put_language(void *priv, char *content)
{
    json_object *new_obj = NULL;  // 声明 JSON 对象
    json_object *parm = NULL;  // 声明参数对象
    json_object *lag = NULL;  // 声明语言对象
    char buf[128];  // 定义字符数组，用于存储信息

    new_obj = json_tokener_parse(content);  // 解析传入的 JSON 字符串
    parm = json_object_object_get(new_obj, "param");  // 获取参数对象
    lag = json_object_object_get(parm, "lag");  // 获取语言对象

    const char *lag_value = json_object_get_string(lag);  // 获取语言值的字符串形式
    printf("cmd_put_language :ctp pro  %s \n", lag_value);  // 打印语言值
    db_update("lag", atoi(lag_value));  // 更新数据库中的语言设置
    lag_set_function(atoi(lag_value));  // 调用外部函数设置语言功能

    json_object_put(new_obj);  // 释放 JSON 对象
    return 0;  // 返回0表示成功
}
...
//设置参数
    ...
    info.fps    = atoi(fps);
    if (atoi(format) == 1) {
        info.type   = VIDEO_FMT_MOV;
    } else if (atoi(format) == 0) {

        info.type = VIDEO_FMT_AVI;
    } else {
        info.type   = VIDEO_FMT_MOV;
...
static int cmd_put_make_forward_files_list(void *priv, char *content)
{
    ...
    case -1 :
        ...
        break;
    case NONE:
        sprintf(buf, "type:0,path:%s"VFLIST_FILE_NAME, get_rec_path_1());
...
static int cmd_put_make_behind_files_list(void *priv, char *content)
{
    ...
    case -1 :
        ...
        break;
    case NONE:
        sprintf(buf, "type:0,path:%s"VFLIST_FILE_NAME, get_rec_path_2());
...
static int cmd_get_ap_ssid_info(void *priv, char *content)
{
    ...
    snprintf(buf, 128, "ssid:%s,pwd:%s", info.ssid, info.pwd);
...
{
    int cnt = 0;
    extern char network_flash_upgrade_state_get(void);
    while (network_flash_upgrade_state_get()) {
        cnt++;
...
static int cmd_put_ctp_cli_disconnect(void *priv, char *content)
{
    ...
    TCP_client_socket_quit(dest_addr->sin_addr.s_addr);
    UDP_client_socket_unreg(dest_addr->sin_addr.s_addr);
...
static int cmd_put_video_bumping(void *priv, char *content)
{
    ...
    sprintf(buf, "w:%d,h:%d,fps:%d,rate:%d,t:%d", 640, 480, net_video_rec_get_fps(), net_video_rec_get_audio_rate(), 10);
    ...
    sprintf(buf, "w:%d,h:%d,fps:%d,rate:%d,t:%d", 1280, 720, net_video_rec_get_fps(), net_video_rec_get_audio_rate(), 10);
    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "VIDEO_BUMPING", "NOTIFY", buf);
    msleep(500);
    net_critical_video_send();
#endif
...
static int cmd_get_pull_video_status(void *priv, char *content)
{
    ...
    switch (res) {
    case VIDEO_RES_1080P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 1280, 720, 1, net_video_rec_get_fps(), net_video_rec_get_audio_rate());
        break;

    case VIDEO_RES_720P:
        sprintf(buf, "w:%d,h:%d,format:%d,fps:%d,rate:%d", 640, 480, 1, net_video_rec_get_fps(), net_video_rec_get_audio_rate());
        break;
...
const struct ctp_map_entry ctp_video_cmd_tab[] sec(.ctp_video_cmd) = {
    ...
    {NULL, "CLOSE_AUDIO_RT_STREAM", NULL, cmd_put_close_audio_rt_stream},
    ...
    {NULL, "CLOSE_PULL_AUDIO_RT_STREAM", NULL, cmd_put_close_pull_audio_rt_stream},
#endif
...
const struct ctp_map_entry ctp_system_cmd_tab[] sec(.ctp_system_cmd) = {
    ...
    {"lag", "LANGUAGE", cmd_get_language, cmd_put_language},  // 注册语言命令，包括获取和设置语言的处理函数

#endif
    {NULL, "FORMAT", NULL, cmd_put_format},
    {"def", "SYSTEM_DEFAULT", NULL, cmd_put_system_default},
