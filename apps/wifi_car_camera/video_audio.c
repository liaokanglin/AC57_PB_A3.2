...
void play_wav_file(const char *file_name, char *sample_source)
{
    union audio_req r = {0};
    if (audio) {
        return;
    ...
    r.dec.priority        = 0;
    r.dec.sample_source = sample_source;//"iis0";
    server_request(audio, AUDIO_REQ_DEC, &r);
...
//生成wav播放文件
void rec_wav_start(char *file_name, u8 audio_type, char *sorce)
{
    union audio_req r = {0};
    struct server *rec  = NULL;
    printf("rec_wav_start fname:%s, audio_type:%d, sorce:%s\n", file_name, audio_type, sorce);
    audio_rec = server_open("audio_server", "enc");
    if (!audio_rec) {
        ASSERT(0, "rec wav audio_rec open faile!");
        return;
    }
    ...
    r.enc.volume          = 63;
    /* r.enc.channel         = 1; */
    r.enc.channel         = 1;//PLNK_CH_MIC_R;
    /* r.enc.channel         = PLNK_CH_MIC_L; */
    r.enc.sample_rate     = 16 * 1000;
    /* r.enc.sample_rate     = 48000; */
    r.enc.wformat         = "pcm";//编码格式为pcm
    /* r.enc.sample_source   = "iis0"; */
    /* r.enc.sample_source   = "iis1"; */
    /* r.enc.sample_source   = "iis2"; */
    /* r.enc.sample_source   = "iis3"; */
    r.enc.sample_source   = sorce;
    /* r.enc.audio_type      = AUDIO_TYPE_ENC_IND;//iis */
    r.enc.audio_type      = audio_type;//iis
    r.enc.callback = audio_callback;
