...
// 定义一个表示视频窗口的结构体
struct video_window {
    uint16_t left;          // 窗口左边缘的位置
    uint16_t top;           // 窗口顶部边缘的位置
    uint16_t width;         // 窗口的宽度
    uint16_t height;        // 窗口的高度
    uint16_t border_left;   // 窗口左边框的宽度
    uint16_t border_top;    // 窗口上边框的宽度
    uint16_t border_right;  // 窗口右边框的宽度
    uint16_t border_bottom; // 窗口下边框的宽度
    uint16_t mirror;        // 是否镜像显示，通常是布尔值（0或1）
    uint16_t rotate;        // 是否旋转显示，通常是布尔值（0或1）
};
