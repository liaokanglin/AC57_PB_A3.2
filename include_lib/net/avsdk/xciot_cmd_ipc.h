...
typedef XCIOT_BYTES_ARRAY_T(524288) ipc_FileThumList_Resp_thum_body_t;
...
typedef struct _ipc_StorageInfo_Resp {
    /** 存储设备当前状态(0: 正常使用; 1: 正在格式化； 2: 未格式化; 3: 存储卡损坏; 4: 未插卡; )*/
    int32_t status;
    /*当只有 status = (0,1,2) 时返回后续两个参数*/
    /** true; 存储设备总大小(MB)*/
    int64_t total_size;
    /** 存储设备使用空间(MB)*/
    int64_t use_size;
