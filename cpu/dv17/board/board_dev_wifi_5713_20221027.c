...
#if  defined CONFIG_BOARD_DEV_WIFI_5713_20221027


#define CONFIG_SENSOR0	1		//0:MIPI, 1:DVP, 2:9930
#define CONFIG_SENSOR1	1		//0:DEP0, 1:DVP1
...
UART1_PLATFORM_DATA_BEGIN(uart1_data)
	...
	/* .baudrate = 115200, */
	 //.tx_pin = IO_PORTH_02,.rx_pin = IO_PORTH_05,
	  .tx_pin = IO_PORTH_11,//.rx_pin = IO_PORTH_15, */
  	.flags = UART_DEBUG,
...
UART3_PLATFORM_DATA_BEGIN(uart3_data)
	...
	.output_channel = OUTPUT_CHANNEL0,
#endif
	.tx_pin =IO_PORTH_14,//  IO_PORTH_06,  //
	//.tx_pin =IO_PORTH_06,//  IO_PORTH_06,  //
  	.flags = UART_DEBUG,
...
{
#define SD_DET_IO   IO_PORTE_05//IO_PORT_PR_03
    static u8 init = 0;
...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
    .lcd_name = "ek79030_v2",
	.lcd_io = {
        .backlight =IO_PORTH_13,// -1,
        .backlight_value = 1,

		.lcd_reset   = IO_PORTH_10,
		.lcd_standby = -1,
        .lcd_cs      = -1,
        .lcd_rs      = -1,
        .lcd_spi_ck  = -1,
    ...
    .lcd_port.mipi_mapping = {
        .x0_lane = MIPI_LANE_EN  | MIPI_LANE_CLK,// MIPI_LANE_D3,
        .x1_lane = MIPI_LANE_EN  | MIPI_LANE_D1,// MIPI_LANE_D2,
...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
    ...
    .lcd_port.mipi_mapping = {
        .x0_lane = MIPI_LANE_EN  | MIPI_LANE_D0 |　MIPI_LANE_EX,
        .x1_lane = MIPI_LANE_EN  | MIPI_LANE_D1　|　MIPI_LANE_EX　,
...
static const struct camera_platform_data camera0_data = {
   ...
   #if 0
    .xclk_gpio      = -1,
	.reset_gpio     =IO_PORTE_01,// IO_PORTH_07,
	.pwdn_gpio      = -1,//IO_PORTH_08,
	.interface      = SEN_INTERFACE0,
	.dvp = {
		.pclk_gpio  = IO_PORTB_05,
		.hsync_gpio =-1,// IO_PORTE_00,
		.vsync_gpio =-1,// IO_PORTE_01,
...
#define CAMERA1_DET -1//IO_PORT_PR_03
    static u8 init = 0;
    	return 1;
    if (!init) {
        init = 1;
...
ADKEY_PLATFORM_DATA_BEGIN(adkey_data)
	.io 		= IO_PORTB_14,//IO_PORTH_10,
	.ad_channel = AD_CH04_PB14,//AD_CH12_PH10,
	.table 	= {
		.ad_value = {
			ADC0_33,
			ADC0_30,
...
WIFI_PLATFORM_DATA_BEGIN(wifi_data)
   ...
   // .sdio_parm = SDIO_GRP_2|SDIO_PORT_1| SDIO_1_BIT_DATA /*|SDIO_DATA1_IRQ */ | 20 * 1000000,
     .sdio_parm = SDIO_GRP_2|SDIO_PORT_1| SDIO_1_BIT_DATA /*|SDIO_DATA1_IRQ */ | 20* 1000000,//20 * 1000000,
...
//    mipi_phy_con0 &= ~BIT(23);//增加这一句 关闭mipi ldo
    ...
    devices_init();
    puts("board_int: over\n");
//    int lcd_init_wait_done();
//    u32 t1 = timer_get_ms();
//    lcd_init_wait_done();
//    printf("\n %s %d %d\n",__func__,__LINE__,timer_get_ms() - t1);
    // sd power on
    /* sdmmc_power(1); */
...
{
    gpio_direction_output(IO_PORTE_04, 0); //让Hi3861L复位
}
...
void doorbell_event_dac_mute(int state)
{
    ...
    state = !state;
    if (old_state != state) {
        gpio_direction_output(IO_PORTG_15, state);
        old_state = state;
    }
}
...
//        delay_2ms(2);  include_lib/liba/dv17/usb.a
